...
//   }
//   await addNoofVolunteers();
// }

const Details = () => {
  const params = useParams();
  const [eventDetails, seteventDetails] = useState(null);
  const [noOfRegisteredUsers, setNoOfRegisteredUsers] = useState(0);
  const eventID = params.id;
  const navigate = useNavigate();

  const addNoofVolunteers = async () => {
    const eventID = params.id;
    try {
      const response = await axios.post("/api/v1/event/addvolunteer/", {
        eventID,
      });
      console.log(response);
      if (response && response.data.success) {
        const updatedVolunteersCount = response.data.event.volunteers;
        // Update the state using a callback function
        setNoOfRegisteredUsers(updatedVolunteersCount);
        console.log(noOfRegisteredUsers);
        toast.success("Volunteer added successfully");
        navigate("/");
      } else {
        toast.error("Volunteer not added");
      }
    } catch (error) {
  ...
  return (
    <div className="slide-container">
      ...
      <p className="place">
        {eventDetails.district}, {eventDetails.state}
      </p>
      <div className="belowSlides">
        <div className="description">
          ...
          <p>no of registered user:{noOfRegisteredUsers}</p>
        </div>
        <div className="eventDetailsButton">
          <Link to={`/event/${eventID}/map`} className="eventDetSubButton">
            VIEW ON MAP
          </Link>
          <Link className="eventDetSubButton" onClick={addNoofVolunteers}>
            REGISTER
          </Link>
        </div>
      </div>
